...
/*   Updated: 2016/11/30 13:40:58 by ybeaure          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_xor(t_vm *vm, t_process *pro, char p_code[4], int p_val[4])
{
	...
	{
	vm->champ[vm->ram[pro->pc].owner].reg[p_val[2] - 1] = get_new_p_val(vm, pro, p_code[0], p_val[0]) ^ get_new_p_val(vm, pro, p_code[1], p_val[1]);
		if (vm->champ[vm->ram[pro->pc].owner].reg[p_val[2] - 1])
			pro->carry = 1;
		else
			pro->carry = 0;
	}
